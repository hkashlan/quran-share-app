...
- [ ] 1. Add DB migration for reading stats columns and RPCs
  - Create `supabase/migrations/004_reading_stats.sql`
...
- [ ] 2. Update TypeScript types and query keys
  - [ ] 2.1 Extend `UserProfile` in `types/khatmah.ts` with `totalJuzLifetime: number`, `totalJuzLastYear: number`, `lastYearResetAt: string | null`
    - _Requirements: 2.1, 1.1_
  - [ ] 2.2 Add `readingStats` key to `profile` namespace in `lib/queryKeys.ts`
    - New entry: `readingStats: (userId: string) => ['profile', userId, 'readingStats'] as const`
    - _Requirements: 5.1, 5.2_

- [ ] 3. Implement Stats_Service and useReadingStats hook
  - [ ] 3.1 Create `lib/stats.ts` with `getReadingStats(userId: string): Promise<ReadingStats>`
  ...
  - [ ] 3.2 Create `hooks/useReadingStats.ts` exporting `useReadingStats()`
...
- [ ] 4. Update Progress_Service to call counter RPCs
  - [ ] 4.1 Update `finishJuz` in `lib/progress.ts` to call `increment_juz_counters` after `awardJazah`
  ...
  - [ ] 4.3 Update `unfinishJuz` in `lib/progress.ts` to fetch the instance first, resolve the awardee, then call `decrement_juz_counters` after reverting the completion flag
    - Call is non-fatal: log warning on error, do not throw
...
- [ ] 5. Update mutation hooks to invalidate readingStats
  - [ ] 5.1 Update `hooks/mutations/useFinishJuz.ts` — add `queryClient.invalidateQueries({ queryKey: queryKeys.profile.readingStats(userId) })` in `onSuccess`
    - _Requirements: 5.2_
  - [ ] 5.2 Update `hooks/mutations/useUnfinishJuz.ts` — add `userId` parameter and invalidate `queryKeys.profile.readingStats(userId)` in `onSuccess`
    - _Requirements: 5.2_

- [ ] 6. Checkpoint — Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Add i18n keys for stats surfaces
...
- [ ] 8. Implement ReadingStatsCard component
  - [ ] 8.1 Create `components/stats/ReadingStatsCard.tsx`
...
- [ ] 9. Implement KhatmahProgressBar component
  - [ ] 9.1 Create `components/khatmah/KhatmahProgressBar.tsx`
...
- [ ] 10. Implement CompletionDonut component
  - [ ] 10.1 Create `components/khatmah/CompletionDonut.tsx`
...
- [ ] 11. Implement JuzGrid component
  - [ ] 11.1 Create `components/khatmah/JuzGrid.tsx`
...
- [ ] 12. Implement InstanceStatsPanel component
  - [ ] 12.1 Create `components/khatmah/InstanceStatsPanel.tsx`
    - Props: `instance: KhatmahInstance`
...
- [ ] 13. Integrate stats components into screens
  - [ ] 13.1 Update `app/(app)/index.tsx` (Dashboard) to use `useReadingStats` and render `ReadingStatsCard` above the Quick Actions section
  ...
  - [ ] 13.2 Update `app/(app)/khatmah/[id]/index.tsx` (KhatmahDetail) to render `InstanceStatsPanel` in `ListHeader` after `KhatmahHeader`
    - Pass the already-fetched `instance` prop
    - _Requirements: 3.1, 3.6, 4.1, 4.5_

- [ ] 14. Create yearly-reset Edge Function
